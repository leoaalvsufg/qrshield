...
import 'package:qrshield/features/report/presentation/widgets/report_card.dart' as widgets;
...
class ReportPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final reportController = ref.read(reportControllerProvider(rawPayload).notifier);
  ...
  Widget _buildBody(BuildContext context, ReportState state, ReportController controller) {
    switch (state.status) {
  ...
  Widget _buildReport(BuildContext context, ReportState state, ReportController controller) {
      ...
      child: Column(
        ...
        children: [
          ...
          widgets.ReportCard(
            ...
            items: riskScore.reasons.map((reason) => ReportItem(
              label: '•',
              value: reason,
            ),).toList(),
            isCollapsible: true,
    ...
    switch (payload) {
      ...
      case final PixPayload pix:
        return [
          ...
          ReportItem(label: 'Checksum', value: pix.crcValid ? 'Válido' : 'Inválido'),
          ReportItem(label: 'Identificador', value: pix.gui.isEmpty ? 'Não encontrado' : pix.gui),
          ...pix.fields.entries.map((entry) => ReportItem(
            label: entry.key,
            value: entry.value,
          ),),
        ];
      ...
      case final WifiPayload wifi:
        return [
          ...
          ReportItem(label: 'Senha', value: wifi.password != null ? '[PROTEGIDA]' : 'Nenhuma'),
          if (wifi.hidden)
      ...
      case final SimplePayload simple:
        return [
          ReportItem(label: 'Tipo', value: _getSimplePayloadTypeName(simple.type)),
  ...
  String _getSimplePayloadTypeName(String type) {
    switch (type) {
      case 'sms': return StringsPt.payloadSms;
      case 'tel': return StringsPt.payloadTel;
      case 'mailto': return StringsPt.payloadEmail;
      case 'geo': return StringsPt.payloadGeo;
      case 'vcard': return StringsPt.payloadVcard;
      case 'text': return StringsPt.payloadText;
  ...
  Widget _buildActionButtons(BuildContext context, ReportState state, ReportController controller) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        // Copy button
          ...
          onPressed: () async {
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(StringsPt.reportCopied),
                ),
              );
            }
          },
  ...
  Future<void> _handleOpenAction(BuildContext context, ReportState state) async {
    final payload = state.payload!;
